...
#define CFS_O_RDWR        (CFS_O_RDONLY | CFS_O_WRONLY)
#define CFS_O_APPEND      (1<<2)
#define CFS_O_CREAT       (1<<3)
#define CFS_O_EXCL        (1<<4)
#define CFS_O_TRUNC       (1<<5)
...
int cfs_read(cfs_t *fs, int fh, void *buf, uint32_t sz);
...
struct cfs_dirent *cfs_readdir(cfs_t *fs, cfs_DIR *d);
